...
/** Subtle overhang for the dock arc (lighter curve than bottom nav). */
export const ARC_UP_SUBTLE_OVERHANG_PX = 12;
...
/** SVG path for a subtle upward arc (used by dock). */
const ARC_UP_SUBTLE_PATH = 'M0,12 Q50,2 100,12 L100,64 L0,64 Z';
...
/** Pre-computed style for up-subtle variant (dock — smaller overhang). */
const arcUpSubtleHeightStyle: React.CSSProperties = { height: `calc(100% + ${ARC_UP_SUBTLE_OVERHANG_PX}px)` };
...
interface ArcBackgroundProps {
  ...
  variant: 'down' | 'up' | 'up-subtle' | 'rect';
...
export function ArcBackground({ variant, className }: ArcBackgroundProps) {
  const path = variant === 'down' ? ARC_DOWN_PATH : variant === 'up' ? ARC_UP_PATH : variant === 'up-subtle' ? ARC_UP_SUBTLE_PATH : RECT_PATH;
  const hasArc = variant !== 'rect';
  const isUpVariant = variant === 'up' || variant === 'up-subtle';

  // "down" and "rect" anchor to the top (arc/content extends downward).
  // "up" variants anchor to the bottom so the arc extends upward above the container.
  const positionClass = isUpVariant
  ...
  return (
    <svg
      ...
      style={hasArc ? (variant === 'up' ? arcUpHeightStyle : variant === 'up-subtle' ? arcUpSubtleHeightStyle : arcDownHeightStyle) : fullHeightStyle}
    >
      ...
      {variant === 'up-subtle' && <path d="M0,12 Q50,2 100,12" fill="none" className="stroke-border" strokeWidth="1" vectorEffect="non-scaling-stroke" />}
    </svg>
  );
}

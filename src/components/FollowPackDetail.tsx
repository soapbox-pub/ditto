...
interface FollowPackDetailPageProps {
  addr: AddrCoords;
}
...
function parsePackEvent(event: NostrEvent) {
  ...
  return { title, description, image, dTag, pubkeys };
}

export function FollowPackDetailPage({ addr }: FollowPackDetailPageProps) {
  const { data: event, isLoading, isError } = useAddrEvent(addr);

  useSeoMeta({
    title: event
      ? `${event.tags.find(([n]) => n === 'title')?.[1] || 'Follow Pack'} - Mew`
      : 'Loading... - Mew',
  });
...
function FollowPackShell({ children }: { children: React.ReactNode }) {
  const navigate = useNavigate();

  return (
    <main className="flex-1 min-w-0 sidebar:max-w-[600px] sidebar:border-l xl:border-r border-border min-h-screen">
      <div className={cn('sticky top-10 sidebar:top-0 z-10 flex items-center gap-4 px-4 h-[53px] bg-background/80 backdrop-blur-md', STICKY_HEADER_CLASS)}>
        <button
          onClick={() => navigate(-1)}
...
function FollowPackContent({ event }: { event: NostrEvent }) {
  const navigate = useNavigate();
  ...
  const handleFollowAll = useCallback(async () => {
    ...
    try {
      // Fetch freshest kind 3 from multiple relays

...
import { GitPullRequest, GitCommit, GitBranch, Copy, Wand2 } from 'lucide-react';
...
export function PullRequestCard({ event, preview = true }: PullRequestCardProps) {
    ...
    <div className="space-y-3 mt-1">
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <span className="font-semibold text-base leading-tight line-clamp-2">{title}</span>
          <div className="flex items-center gap-2 mt-1 text-sm text-muted-foreground">
            {repoName && <span>{repoName}</span>}
            {repoName && branchName && <span className="text-muted-foreground/50">/</span>}
            {branchName && (
              <Badge variant="secondary" className="text-xs font-mono px-2 py-0">{branchName}</Badge>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {branchName && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <code className="bg-muted px-2 py-0.5 rounded text-xs font-mono">{branchName}</code>
            </div>
          )}
          {commitTip && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <code className="bg-muted px-2 py-0.5 rounded text-xs font-mono">{commitTip.slice(0, 12)}</code>
            </div>
          )}
          {mergeBase && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <code className="bg-muted px-2 py-0.5 rounded text-xs font-mono">{mergeBase.slice(0, 12)}</code>
            </div>
          )}
        </div>
      )}

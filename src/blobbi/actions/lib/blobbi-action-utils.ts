 ...
 */
export type InventoryAction = 'feed' | 'play' | 'clean';
...
/**
 * Stages that can use inventory items (food, toys, hygiene)
 */
export const ITEM_USABLE_STAGES = ['baby', 'adult'] as const;

/**
 * Check if a companion can use inventory items.
 * Eggs cannot use food, toys, or hygiene items.
 */
export function canUseInventoryItems(companion: BlobbiCompanion): boolean {
  return ITEM_USABLE_STAGES.includes(companion.stage as typeof ITEM_USABLE_STAGES[number]);
}

/**
 * Get a user-friendly message explaining why items can't be used.
 */
export function getStageRestrictionMessage(companion: BlobbiCompanion): string | null {
  if (companion.stage === 'egg') {
    return 'Eggs cannot use items. Wait for your Blobbi to hatch!';
  }
  return null;
}

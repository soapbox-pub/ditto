...
export function RoomFurnitureLayer({
  ...
  // Sort within each layer by renderOrder so rugs/beds always render behind default items
  for (const layer of ['back', 'floor', 'front'] as const) {
    grouped[layer].sort((a, b) => {
      const orderA = resolveFurniture(a.placement.id)?.renderOrder ?? 0;
      const orderB = resolveFurniture(b.placement.id)?.renderOrder ?? 0;

...
export interface TarotReading {
  /** The three drawn cards in past/present/future order (empty while loading or logged out). */
  cards: TarotCardData[];
...
/**
 * Manages one tarot reading (daily or weekly) for the logged-in user: restores
 * a sealed reading from their shared Nostr note or the local cache, or draws a
 * fresh spread. Sealing (revealing all three cards) persists the reading
 * locally until the window expires. Without a user, no cards are dealt.
 */
export function useTarotReading(type: ReadingType): TarotReading {
  const { user } = useCurrentUser();
  const pubkey = user?.pubkey;
  const cacheKey = pubkey ? readingCacheKey(type, pubkey) : null;
  ...
  const writeCache = useCallback(
    (reading: CachedReading) => {
      try {
        if (cacheKey) localStorage.setItem(cacheKey, JSON.stringify(reading));
      } catch {
  ...
  useEffect(() => {
    ...
    const cached = cacheKey ? readCachedReading(cacheKey) : null;
    const validFortune =
  ...
  }, [type, pubkey, isLoading, fortune, cacheKey, writeCache, clearCache]);

  // Seal the reading once all three cards are face-up.
  useEffect(() => {
    if (cards.length === 3 && revealed.every(Boolean) && !sealed) {
      setSealed(true);
      if (cacheKey && !readCachedReading(cacheKey)) {

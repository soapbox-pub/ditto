 ...
 * settings schema, full Lua source (in a collapsible), and install /
 * uninstall / open affordances. The `naddr` param is a NIP-19
...
export function TileDetailPage() {
  ...
  return (
    <main className="pb-16 sidebar:pb-0">
              ...
              {installed
                ? (
                  <>
                    <Button asChild>
                      <Link
                        to={`/tiles/run/${encodeURIComponent(parsed.identifier)}`}
                      >
      ...
      {/* Reference to keep nip19 import alive even if we don't use it
          directly here (we already decoded via decodeTileNaddr). */}
      {nip19 && null}
    </main>
  );
}

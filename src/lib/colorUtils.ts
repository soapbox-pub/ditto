...
/** Check if foreground/background HSL strings meet WCAG AA (4.5:1). */
export function meetsAA(fgHsl: string, bgHsl: string): boolean {
  const fg = parseHsl(fgHsl);
  const bg = parseHsl(bgHsl);
  const fgRgb = hslToRgb(fg.h, fg.s, fg.l);
  const bgRgb = hslToRgb(bg.h, bg.s, bg.l);
  return getContrastRatio(fgRgb, bgRgb) >= 4.5;
}

...
function replaceBlocked(next: Set<string>): void {
  const changed = [
    ...[...next].filter((url) => !blocked.has(url)),
    ...[...blocked].filter((url) => !next.has(url)),
  ];
  blocked = next;
  if (changed.length) {
...
/**
 * Subscribe to changes in the blocked set, with the normalized URLs that were
 * blocked or unblocked. The pool uses this to drop connections it already has.
 */
...
/**
 * Relays that can't be blocked: the relays of logged-in accounts' remote
 * signers (NIP-46), without which signing would silently stop working.
 */
let exempt = new Set<string>();

/** Set the relays that can't be blocked. See {@link exempt}. */
export function setUnblockableRelays(urls: string[]): void {
  exempt = new Set(urls.map(normalizeRelayUrl).filter((u): u is string => !!u));
}
...
export function setBlockedRelays(pubkey: string | undefined, urls: string[], privateUrls?: string[]): void {
  replaceBlocked(new Set(urls.map(normalizeRelayUrl).filter((u): u is string => !!u)));
  if (!pubkey) return;
  try {
...
export function loadBlockedRelays(pubkey: string | undefined): void {
  replaceBlocked(new Set(pubkey ? readBlockedCache(pubkey).relays : []));
}
...
export function isRelayBlocked(url: string): boolean {
  ...
  return normalized !== undefined && blocked.has(normalized) && !exempt.has(normalized);
}
...
/**
 * `NRelay1` that can tell when the relay actually requires AUTH. Many relays
 * send a challenge as soon as a socket opens, even when nothing needs it;
 * {@link AuthAwareRelay.authNeeded} resolves only once the relay refuses a
 * request with an `auth-required:` reply, so a prompt can wait until then.
 * Nostrify retries that request after AUTH completes.
 */
export class AuthAwareRelay extends NRelay1 {
  ...
  protected override receive(msg: NostrRelayMsg): void {
    // A new challenge (e.g. after a reconnect) starts a new wait.
    if (msg[0] === 'AUTH') this.needed = deferred();
    if (
